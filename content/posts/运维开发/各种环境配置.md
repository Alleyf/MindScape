...
title: "各种环境配置"
date: "2026-03-08"
tags: ["Java","MySQL","Nacos","中间件"]
personality: "引路人"
description: "各种开发环境与中间件配置教程"
cover: "/images/covers/ms-530caf31.svg"
---
...
[CentOS7下安装mysql5.7](https://blog.csdn.net/ic_xcc/article/details/121425926)
...
![](http://img.fcs.cloudns.ch/pics/202311280029568.png)
![](http://img.fcs.cloudns.ch/pics/202311280030655.png)
...
参考：
- [Linux下搭建Mysql主从复制详细步骤（Mysql版本5.7.30）](https://blog.csdn.net/cool_summer_moon/article/details/106112560)
- [linux mysql5.7主从配置 详细步骤，按步骤绝对能够成功](https://blog.csdn.net/qq_42236784/article/details/136500477)
...
3. 连接主库（mysql8需要加上get_master_public_key=刚才设置的server-id）：
...
![|400](http://img.fcs.cloudns.ch/pics/202407212335679.png)
...
参考：[MySQL同步故障：" Slave\_SQL\_Running:No" 两种解决办法-CSDN博客](https://blog.csdn.net/Heng_Ji/article/details/51013710)
...
1. windows 下载安装 MongoDB
   ...
   ![|500](http://img.fcs.cloudns.ch/pics/202312141347269.png)
...
![|600](http://img.fcs.cloudns.ch/pics/202312141356241.png)
...
![](http://img.fcs.cloudns.ch/pics/202312101238746.png)
...
![](http://img.fcs.cloudns.ch/pics/202312181916609.png)
...
![](http://img.fcs.cloudns.ch/pics/202311271638947.png)
...
![](http://img.fcs.cloudns.ch/pics/202311271638364.png)
...
在**不损坏已有镜像、容器、卷、网络**的前提下，把 **CentOS 7.9** 上的 **Docker 降级到 18.09.8**，核心思路是：

> **只卸载 rpm 包，不动 `/var/lib/docker`；随后安装旧版 rpm，守护进程重启后会自动识别原有数据。**
...
```bash
# 备份镜像/容器元数据
docker system df                    # 看一眼当前资源
tar -czf /var/tmp/docker-lib-$(date +%F).tar.gz -C /var/lib docker
```
...
> 注意：  
> - **不要加 `containerd.io`**，新版与 18.09.8 兼容，可继续沿用；  
> - **不要手动删除 `/var/lib/docker`**，否则镜像/容器会丢失。
...
docker version            # 服务端应显示 18.09.8
docker images             # 原有镜像应全部可见
docker ps -a              # 原有容器应全部列出
```
...
sudo yum versionlock add docker-ce-18.09.8 docker-ce-cli-18.09.8
```
...
| 场景 | 处理 |
|---|---|
| 启动报错 `graphdriver` / `overlay2` 不兼容 | 18.09.8 与 20.x 使用同一 `overlay2` 格式，一般可直接识别；若曾启用实验特性，可 `dockerd --storage-driver=overlay2` 强制指定。 |
| 容器无法启动 | 先 `docker start <container>`，若失败看 `docker logs <container>`；99% 是容器内应用问题，与降级无关。 |
| 使用 `devicemapper` | 18.09 同样支持，无需转换。 |

### ✅ 一句话总结

**只要你不删 `/var/lib/docker`，卸载再装回 18.09.8 的 rpm，所有镜像、容器、卷、网络都会完好无损地回来。**
...
![image.png](http://img.fcs.cloudns.ch/pics/202311181541807.png)
...
![image.png](http://img.fcs.cloudns.ch/pics/202311181541977.png)
---
...
![image.png](http://img.fcs.cloudns.ch/pics/202311201527195.png)
...
![image.png](http://img.fcs.cloudns.ch/pics/202311201526596.png)
5. 导入初始配置到 nacos
...
![image.png](http://img.fcs.cloudns.ch/pics/202311201533687.png)
6. 启动 seata
根据系统环境启动 seata 服务端：
![image.png|500](http://img.fcs.cloudns.ch/pics/202311201535684.png)
---
...
![image.png](http://img.fcs.cloudns.ch/pics/202311221729835.png)
...
![](http://img.fcs.cloudns.ch/pics/202312101120616.png)

![|800](http://img.fcs.cloudns.ch/pics/202312101118368.png)
...
![](http://img.fcs.cloudns.ch/pics/202312102334878.png)

![|500](http://img.fcs.cloudns.ch/pics/202312102334410.png)
...
![](http://img.fcs.cloudns.ch/pics/202312102335214.png)
...
![](http://img.fcs.cloudns.ch/pics/202312102340085.png)
---
...
-e USER_AUTHORIZATION="YOUR_DISCORD_USER_AUTH_HERE" \
-e TZ=Asia/Shanghai \
...
services:
  coze:
    ...
    environment:
      ...
      - USER_AUTHORIZATION=YOUR_DISCORD_USER_AUTH_HERE  # 必须修改为我们 discord 用户的授权密钥
      - BOT_TOKEN=YOUR_DISCORD_BOT_TOKEN_HERE  # 必须修改为监听消息的 Bot-Token
...
![image.png](http://img.fcs.cloudns.ch/pics/202311182136214.png)
...
`df`命令在Linux系统中用于显示文件系统的磁盘空间使用情况。`-h`是`df`命令的一个选项，它代表“human-readable”，意味着输出将以易于阅读的格式显示，例如以KB、MB、GB等单位，而不是默认的块（block）大小。

使用`df -h`时，你会得到以下类型的信息：
...
在这个例子中，`/dev/sda1`是一个磁盘分区，总大小为100GB，已使用20GB，剩余80GB，使用了20%的空间，并且挂载在根目录`/`上。`tmpfs`是一个临时文件系统，通常用于存储临时文件，大小为3.9GB，尚未使用任何空间。
