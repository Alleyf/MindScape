...
export function NoteCard({ note, index }: NoteCardProps) {
  ...
  return (
    <motion.div
      ...
      className="glass-card p-0 cursor-pointer relative overflow-hidden group"
    >
      ...
      <Link to={`/note/${note.slug}`} className="relative z-10 flex flex-col md:flex-row md:items-stretch note-card-link">
        ...
        <div className="p-6 flex-1 min-w-0 flex flex-col">
          ...
          <h3 className="text-xl font-bold mb-3 gradient-text line-clamp-2">{note.title}</h3>

          <p className="theme-muted text-sm mb-4 line-clamp-3 leading-relaxed">{note.excerpt}</p>
          ...
          <div className="flex items-center justify-between text-xs theme-subtle mt-auto">
            {note.personality && (
